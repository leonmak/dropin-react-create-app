...
CommentsController.getUserComments = function(req, res) {
  ...
  Comments.where({user_id: user_id}).fetchAll().then(function(comments) {
    res.json(comments.toJSON());
  }).catch(function(err) {
    res.json({error: MESSAGES.ERROR_USER_COMMENT_NOT_FOUND});
  })
}
...
CommentsController.getComment = function(req, res) {
	...
	Posts.where({post_id: post_id, id: id}).fetch().then(function(post) {
    res.json(post.related('comments').where({id: id}).fetch().then(function(comment) {
...
CommentsController.directComment = function(userId, postId, text, res = null) {
  ...
  if (user.anonymous == false) {
	...
	new Comments().save(commentHash).then(function(comment) {
		...
		if (res !== null) {
			res.json({error: MESSAGES.ERROR_POSTING_COMMENT});
		}
	});
}
